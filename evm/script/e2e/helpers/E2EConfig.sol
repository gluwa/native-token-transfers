...
abstract contract E2EConfig is Script {
    ...
    function encodeSpecialRelayInstructions(
        ...
        returns (bytes memory encoded, TransceiverStructs.TransceiverInstruction memory instruction)
    {
        ...
        instruction = TransceiverStructs.TransceiverInstruction({
            index: 0, payload: transceiver.encodeWormholeTransceiverInstruction(wormholeInstruction)
        });
    ...
    function readChainDeployment(
        string memory chainKey
    ) internal view returns (ChainDeployment memory dep) {
    ...
    function _formatSrcExports(
        ChainDeployment memory dep
    ...
    function parseVaaEmitterChainId(
        bytes memory vaa
    ...
    function parseVaaEmitterAddress(
        bytes memory vaa
    ...
    function assertWormholeCoreChainId(
    ...
    function validateSrcDeployment(
        ...
        address wormholeCore = address(WormholeTransceiverState(src.wormholeTransceiver).wormhole());
        assertWormholeCoreChainId(wormholeCore, src.wormholeChainId);
    }

    function validateDstDeployment(
        ...
        address wormholeCore = address(WormholeTransceiverState(dst.wormholeTransceiver).wormhole());
        assertWormholeCoreChainId(wormholeCore, dst.wormholeChainId);
    }

    function validateDstAcceptsSrcVaa(
        ChainDeployment memory src,
        ChainDeployment memory dst
    ) internal view {
    ...
    function assertSignedVaaMatchesSrc(
